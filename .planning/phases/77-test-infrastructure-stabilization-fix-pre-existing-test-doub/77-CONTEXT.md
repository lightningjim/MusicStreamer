...
- **D-01:** **All six clusters are in scope for this phase.** The roadmap goal — "clean full-suite `pytest tests/` run" — implies no failure left behind from the named list. Splitting into multiple phases would defeat the goal.

- **D-02:** **Bar = `pytest tests/` exits 0 with no `xfail` / `skip` masking** for the six named clusters. If a fix genuinely cannot land in Phase 77 (e.g., needs a CI sandbox change), `<discretion>` allows the planner to scope-defer that one item with explicit `<deferred>` entry — but the default is "fix in this phase."

- **D-03:** **Pre-existing failures NOT in the six-cluster list stay deferred.** If the planner discovers a 7th failure cluster during scope-lock, it goes into `<deferred>` for a follow-up phase. Don't silently expand scope.
...
- **D-05:** **`set_plugin_option` cluster (cluster 6): impl follows test.** The Phase 31 commit-history intent (cited verbatim in `tests/test_twitch_auth.py:1-9`) was the scoped `session.set_plugin_option("twitch", "api-header", ...)` form. Production at `musicstreamer/player.py:1156` should switch from `session.set_option("twitch-api-header", ...)` to the scoped form. **Planner verifies the streamlink version pinned in `pyproject.toml` actually supports `set_plugin_option`** — if a version bump is required, the planner makes that decision with the rest of the fix.
...
- **D-08:** **Canonical signal set = every `Signal(...)` declared on `musicstreamer.player.Player`.** Drift-guard test imports both `musicstreamer.player.Player` and the shared `_FakePlayer`, walks `Player.__dict__` for `Signal` attributes, asserts every one is also present on `_FakePlayer`. New Player signals fail loudly the moment they're added without a parity update. This mirrors the project's established "source-grep / source-introspection drift-guard" convention (memory: `feedback_gstreamer_mock_blind_spot.md`; Phase 79 Plan 04; Phase 69 Plan 05).

- **D-09:** **All 12 existing fake-player sites switch to the shared import.** Planner enumerates them; the list at scout time is: `tests/test_stream_picker.py`, `tests/test_discovery_dialog.py`, `tests/test_ui_qt_scaffold.py`, `tests/test_main_window_integration.py`, `tests/test_equalizer_dialog.py`, `tests/test_main_window_media_keys.py`, `tests/test_now_playing_panel.py`, `tests/test_main_window_soma.py`, `tests/test_phase72_now_playing_panel.py`, `tests/test_main_window_gbs.py`, `tests/test_phase72_1_stream_picker_reflow.py`, `tests/ui_qt/test_main_window_node_indicator.py`. Each site loses its local class definition; tests import `from tests._fake_player import FakePlayer` (or whatever module name the planner picks — see `<discretion>`).
...
- **D-12:** **Block real network calls in tests as the primary fix for the `test_first_call_shows_toast` crash.** Root cause is a real `urllib.urlretrieve` call inside an EditStationDialog logo-fetch worker thread that lives past parent widget GC. Planner picks one of: (a) `monkeypatch.setattr(urllib.request, "urlretrieve", ...)` per test, (b) install `pytest-socket` and add `disable_socket = true` to `pyproject.toml`'s `[tool.pytest.ini_options]`, with an explicit allowlist for the few legitimately-networked integration tests. Recommendation: option (a) for surgical local fix; (b) if planner judges global protection is worth the dependency.

- **D-13:** **Cover-art worker thread teardown.** The `test_main_window_integration.py → test_now_playing_panel.py` and `test_phase72_now_playing_panel.py → test_phase72_assumptions.py` crashes share a common signature with the Plan 72-02 "cover-art worker thread, signal-source-deleted race" diagnosis. Planner fixes this at the **production-code layer** (cover-art worker awaits `wait()` before parent-widget destructor, or unparents itself in `finished`), not via test-runner trickery. Mirrors Phase 68's existing cover-art worker pattern (`closeEvent` calls `worker.wait(16000)`).
...
- **D-15:** **`QStackedWidget` visibility under `offscreen` platform** (cluster 5b). Planner reproduces, then picks one of: (a) `panel.show()` before the `isVisibleTo` check, (b) replace `isVisibleTo(panel)` with `panel._stack.currentIndex() == 0` (semantic equivalent — search box is on page 0, so its container-page-active state IS its effective visibility), (c) `qtbot.waitExposed(panel)` before the assertions. Option (b) is the most robust under offscreen; option (a) is the smallest diff.

### Drift-guard regression test (locks the recurrence shape)

- **D-16:** **Drift-guard pytest in `tests/test_fake_player_signal_parity.py`.** Imports `musicstreamer.player.Player`, walks `Player.__dict__` for `Signal`-typed attributes (use `isinstance(v, type) and v.__name__ == "Signal"` or the Shiboken type-check), then imports `tests._fake_player.FakePlayer` and asserts every Player signal name is present (as a `Signal`-typed attribute) on FakePlayer. Fails loudly the first time someone adds a new Player signal without parity.
