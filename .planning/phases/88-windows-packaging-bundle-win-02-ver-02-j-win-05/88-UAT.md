...
[testing paused — Test 6 issue (QtWebEngine not bundled, exit=2); Tests 7/8 blocked on same root cause. Awaiting VM diagnostic to fork env-provisioning vs spec-fix.]
...
result: issue
reported: "\"Login helper crashed unexpectedly\" exit=2"
severity: blocker
...
result: issue
reported: "User confirmed Twitch also fails to launch the web login (same exit=2 class)"
severity: blocker
...
result: issue
reported: "User confirmed Google/YouTube also fails to launch the web login (same exit=2 class)"
severity: blocker
