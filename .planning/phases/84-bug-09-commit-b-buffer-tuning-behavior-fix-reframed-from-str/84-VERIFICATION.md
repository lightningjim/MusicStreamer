...
- **D-12** — always-visible `Buf duration` stats-for-nerds row + `buffer_duration_changed = Signal(int, bool)` carrying `(seconds, is_adapted)` (payload widened by code-review WR-02 fix to decouple the "(adapted)" suffix from a `BUFFER_DURATION_S`-equality compare) wired through `MainWindow` DirectConnection to `NowPlayingPanel.set_buffer_duration`.
...
| 3  | `buffer_duration_changed = Signal(int, bool)` declared at `Player` class scope (D-11 / D-12 Signal contract; structurally mirrors Phase 78 `underrun_count_changed` wiring pattern, but with a wider `(seconds, is_adapted)` payload per code-review WR-02 fix in commit `1d594b1`) | VERIFIED | `musicstreamer/player.py:306` immediately after `underrun_count_changed = Signal(int)` (Phase 78 mirror at :297, narrower payload — unchanged); shipped in commit `29d0dea` as `Signal(int)`, widened to `Signal(int, bool)` in WR-02 fix commit `1d594b1`; `tests/test_player_buffer_growth.py::test_buffer_duration_changed_signal_at_class_scope` GREEN. |
...
| `tests/_fake_player.py` | INFRA-01 parity mirror: `buffer_duration_changed = Signal(int, bool)` (FakePlayer leads Player in Wave 0 with the original `Signal(int)`, then re-paritied to `Signal(int, bool)` in code-review WR-02 fix `1d594b1`) | VERIFIED | Plan 01 commit `d4ab335` added the original `Signal(int)` mirror + docstring count clarification; WR-02 fix `1d594b1` widened FakePlayer parity to match the new Player payload; `tests/test_fake_player_signal_parity.py` 2/2 GREEN throughout Wave 0, Wave 1, and post-WR-02. |
