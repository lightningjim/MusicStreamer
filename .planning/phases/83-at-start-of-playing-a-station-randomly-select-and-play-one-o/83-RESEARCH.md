...
1. **Does playbin3 reliably fire `about-to-finish` for ~5-8s clips in MusicStreamer's specific configuration (buffer-duration=10s, buffer-size=10MB, GST_PLAY_FLAG_BUFFERING=0x100)?**
   ...
   - **RESOLVED 2026-05-22 via live spike** on Linux GStreamer 1.28.2 with MusicStreamer's exact flags (`flags |= 0x100`, `buffer-duration = 10s`, playbin3, Beat Blender preroll → beatblender-128-mp3 stream). Spike timeline: `about-to-finish` fires very early in the preroll's lifecycle (playbin3 pre-queues the next URI ahead of EOS, as designed for gapless playback). Pipeline transitions to PLAYING, plays the preroll, and the new STREAM_START for the queued station-stream URI fires at **+7.849s** — matching the preroll's 7.99s duration to within decode jitter. No `message::eos` is emitted on the normal path (the live stream after the preroll does not EOS). **Gapless handoff via `about-to-finish` + set next URI works as intended for ~8s prerolls under MusicStreamer's buffer flags.** No need to drop the buffer flag for the preroll URI. Manual UAT still recommended as a regression sentinel — see VALIDATION.md.
...
2. **What does playbin3 do if the about-to-finish handler sets `uri` to a YouTube/Twitch URL that requires async resolution?**
   ...
   - **RESOLVED:** acceptable per CONTEXT.md D-05's Claude's-Discretion clause — the worst case is "preroll ends, then a brief silence, then the YT stream starts." That's still better than the current "skip preroll entirely" baseline. SomaFM stations do not have YouTube secondary streams in practice (the catalog is direct ICE relays), so this is a paper edge case. No plan-side mitigation required; covered by D-09 silent-failure path if the resolver outpaces playbin3's EOS.
...
3. **Does the project's bus handler set already cover EOS, or only `error`, `tag`, `buffering`, `state-changed`?**
   ...
   - **RESOLVED 2026-05-22 via live spike (above) + planner decision:** The normal-path spike confirmed no `message::eos` is reached for ~8s preroll → live stream handoff. For the malformed-preroll edge case, the planner adds a behavioral test (`test_preroll_eos_without_about_to_finish_advances_to_stream`) to 83-03's appendix that simulates a bus EOS message while `_preroll_in_flight` and asserts `_try_next_stream` is invoked. The minimal `message::eos` connection (`bus.connect("message::eos", self._on_gst_eos_during_preroll)` gated on the preroll flag) is added inside the existing bus-handler setup. Single-line connection + single-method handler, no new state machinery.
...
4. **Is the `random.choice` selection seed-able for deterministic tests?**
   ...
   - **RESOLVED:** covered by `monkeypatch.setattr(random, "choice", ...)` in tests. No code-side change required.
