...
|---|-------|---------|---------------|
| A1 | `QSplitter.hide()` on a child auto-hides the adjacent handle in PySide6 6.10+ | Pattern 1, Don't Hand-Roll | LOW — even if it doesn't, a one-line `self._splitter.handle(1).hide()` is a trivial addition. Plan-time test will lock current behavior. [Source: forum.qt.io/topic/45377 is an older Qt 4/5 thread; explicit Qt 6 docs are silent — empirical verification needed in Wave 0 spike test.] |
