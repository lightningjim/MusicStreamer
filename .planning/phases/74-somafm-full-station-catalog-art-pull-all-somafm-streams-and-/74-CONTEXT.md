...
- **Quality tier mapping.** SomaFM's `channels.json` returns multiple `playlists` per channel, each with `format` (mp3/aac/aacp) and `quality` (highest/high/low). Map to existing `quality` field ("hi"/"med"/"low") OR leave blank and let `stream_ordering.py` codec+bitrate sort handle ordering. Planner picks — recommend the explicit hi/med/low mapping for AA-pattern consistency.
