...
result: pass
note: "User confirmed a dead station still shows 'Stream exhausted' after the YouTube->direct edit in the same session — gate did not leak. (A 'Playback error' GStreamer toast appeared for the unplayable direct URL, which is expected and distinct from the failover 'Stream exhausted' toast.)"
...
result: pass
note: "User tested both variants — play A fully then switch to B, and switch to B before A finishes loading. Both clean, no spurious toast."
