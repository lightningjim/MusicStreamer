...
- **D-03:** Use the official MSVC runtime build from gstreamer.freedesktop.org, latest 1.24.x release. Planner pins the exact version during research and records it in findings.
  - **Amendment 2026-04-20 (a):** Bumped to **1.28.2** (current latest, 2026-04-08). 1.28.x ships as a single `.exe` installer — the former split runtime/devel MSI layout (1.24/1.26) was consolidated upstream. Downstream impact: build.ps1 no longer checks a separate `$GstDevel` path, and README install flow uses the Complete feature-set wizard instead of two `msiexec /qn` calls.
  - **Amendment 2026-04-20 (b):** Spike source of GStreamer relaxed to **conda-forge** as the primary path; MSVC installer is now the Alternative. Rationale: PyGObject ships no Windows wheels on PyPI, so the MSVC-installer route required ~1 GB of Visual Studio Build Tools + meson/ninja + pkg-config plumbing to build PyGObject from source against the MSI's devel headers — too much yak-shaving for a throwaway spike. Conda-forge's GStreamer/PyGObject packages are built by the same upstream gvsbuild toolchain that produces the official MSI, so Phase 44 findings transfer with cosmetic DLL-name differences only. The spike now produces findings for the conda-forge tree; Phase 44 can either inherit directly or, if MSI distribution is hard-required, port the findings (same layout, same DLL set minus prefix).
