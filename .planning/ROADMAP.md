...
### Phase 999.7: yt-dlp overwrites cookies.txt — regression of FIX-02 (BACKLOG — BUG)

**Goal:** After Google login imports Netscape-format cookies via `oauth_helper._GoogleWindow`, playing a YouTube stream causes yt-dlp to rewrite `~/.local/share/musicstreamer/cookies.txt` with its own serialization (file now starts with `# This file is generated by yt-dlp.  Do not edit.` instead of `# Netscape HTTP Cookie File`). This breaks subsequent logins and playback — the imported cookies are lost on first yt-dlp invocation. v1.5 FIX-02 specifically addressed this by using temporary copies of cookies.txt for yt-dlp/mpv invocations; something regressed the temp-copy protection. Reproduced 2026-04-24 during Phase 999.3 UAT. Likely culprits to investigate: `yt_import.py`, `player.py` yt-dlp invocations, any recent change to `paths.cookies_path()` consumers.
**Requirements:** TBD
**Plans:** 0 plans
